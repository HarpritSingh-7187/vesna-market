...
// Exploration loop: find an unvisited region and go there
+!explore : region(R) & not visited(R) <-
    .print("Next stop: ", R);
    !visit(R);
    !explore.

+!explore : not (region(R) & not visited(R)) <-
    .print("I have visited all regions! Exploration complete.").
...
+!visit(R) : true <-
    ...
    .print("Walking to ", R, "...");
    vesna.walk(R);
    // Wait for the movement completion signal
